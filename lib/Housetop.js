...
function Housetop (options, debug) {
  ...
  var self = this;
  var material = [];
  var finalPatch = options.initialPatch || null;
  ...
  this.render = function (knownSize, gutterWidth, knownDimension, renderFn) {
    knownDimension = knownDimension || DIMENSION.w;

    /*
     * if you pass a render function
     * but don't want to pass knownDimension
     */
    if (typeof arguments[2] === 'function') {
      renderFn = knownDimension;
      knownDimension = undefined;
    }
    ...
    var resolved = $.resolvePatch(finalPatch, x, gutterWidth, renderFn);
