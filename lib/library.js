 ...
 * Normalize patches
 *
 * takes a group of patch objects and 
 * sets the target dimension the same for all
 ...
 * Sum Patches
 *
 * computes the total size in a given dimesion
 *
 */
 ...
 *
 * Patch argument can be
 * a number (takes patch objects from the supply)
 * an array of patch objects
 * or a single patch object
 *
 * this function normalizes the output to a patch array
 * 
 */
...
function prependPatch (currentPatch, newPatch, orientation) {
  ...
  currentPatch[dimension] = add(add(currentPatch[dimension],newPatch[dimension]),{x:0,g:1});
...
function defaultRender (patch, dimensions) {
  return _.extend({}, patch, dimensions);
}
 
function resolvePatch (patch, x, gutter, render, offset) {
  ...
  var finalPatches = []
    , w = computeRealSize(patch.w, x, gutter)
    , h = computeRealSize(patch.h, x, gutter)
    ;
  
  if(!patch.patches) {

    /*
     * this is a leaf patch
     */
    finalPatches.push(render(patch, {
      w: w,
      h: h,
      x: offset.x || 0,
      y: offset.y || 0
    ...
    var childOffset = { x: offset.x, y: offset.y };

    _.each(patch.patches, function (child) {

      /*
       * add the child final patches
       */
      var resolvedChildren = resolvePatch(child, x, gutter, render, childOffset);
      finalPatches = finalPatches.concat(resolvedChildren.patches);

      /*
       * track how the child affects current position
       */
      if(patch.orientation === ORIENTATION.horizontal) {
        childOffset.x += (computeRealSize(child.w, x, gutter) + gutter);
      } else {
        childOffset.y += (computeRealSize(child.h, x, gutter) + gutter);
      }

    });

    // // Todo: maybe fix the rounding errors

    // var roundingError;

    // if (patch.orientation === ORIENTATION.horizontal) {
    //   roundingError = childOffset.x - (w + gutter);  
    // } else {
    //   roundingError = childOffset.y - (h + gutter);
    // }
