...
function roundComputedSize (computed, precision) {
  precision = precision || 5;
  return parseFloat( computed.toFixed(precision) );
}

function computeRealSize (virtualSize, x, gutter) {
  var size = (virtualSize.x * x) + (virtualSize.g * gutter);
  return roundComputedSize(size);
}
...
function vallidatePatch (patch) {
  ...
  if(normalized && 
      roundComputedSize(sum,4) === roundComputedSize(patch[dimension],4) &&
